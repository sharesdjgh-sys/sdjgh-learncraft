...
import { generateReviewedLearningIllustration, illustrationReviewSchema, illustrationReviewPassed, type IllustrationBrief } from "../src/lib/learning-image-quality";
...
const pass = illustrationReviewSchema.parse({ meaningPreserved: true, issues: [] });
...
async function main() {
  ...
  const generate: Parameters<typeof generateReviewedLearningIllustration>[1] = {
    generate: async input => {
      ...
      if (generations === 2) assert(input.prompt.includes("증발과 응결의 방향을 바로잡기"));
    ...
    review: async input => {
      ...
      return { review: reviews === 1 ? { ...pass, meaningPreserved: false, issues: ["증발과 응결의 방향을 바로잡기"] } : pass, usage: {} as never };
    },
  };
  const stages: string[] = [];
  ...
  for (const issue of ["한글 일부가 깨졌으나 의미는 유지됨", "원문과 다른 표현이나 같은 의미", "부가 설명 일부 생략", "장식적인 아이콘과 작은 글자"]) {
    const acceptable = { meaningPreserved: true, issues: [issue] };
    assert(illustrationReviewPassed(acceptable));
    let attempts = 0;
    const accepted = await generateReviewedLearningIllustration(args, { ...generate,
      review: async () => { attempts++; return { review: acceptable, usage: {} as never }; },
    });
    assert.equal(attempts, 1, "Cosmetic defects must not trigger regeneration");
    assert.equal(accepted.attempts, 1);
  }
  for (const issue of ["증발과 응결의 방향이 반대", "수식의 부호가 바뀌어 의미 왜곡", "그림의 대상 이름이 뒤바뀜"]) {
    let attempts = 0;
    await assert.rejects(generateReviewedLearningIllustration(args, { ...generate,
      review: async () => { attempts++; return { review: { meaningPreserved: false, issues: [issue] }, usage: {} as never }; },
  ...
  console.log("PASS: meaning-only review, cosmetic defects accepted without retry, distorted concepts corrected/rejected, encoded-image review, reviewer outage and cancellation");
}
void main();
