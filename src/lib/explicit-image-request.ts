export const EXPLICIT_IMAGE_FAILURE = "요청하신 Nano Banana 이미지를 생성하거나 이미지 처리를 완료하지 못했어요. 다시 요청해 주세요.";
...
- 도구가 scheduled:true를 반환하면 이미지 생성은 별도로 진행 중입니다. 기다리지 말고 핵심 개념과 원리를 본문으로 먼저 충분히 설명합니다. 실제 그림을 보았거나 완성됐다고 말하지 않습니다. 성공한 이미지는 이미지 처리 후 앱이 답변에 표시합니다. 도구가 placementMarker를 반환하면 관련 설명 직후의 독립된 줄에 그 표식을 정확히 한 번 출력하고 설명을 이어갑니다. 코드 블록과 별도 이미지는 출력하지 않습니다.
- 실패하면 Nano Banana 이미지 생성 또는 처리 실패를 정확히 알리고 다시 요청하도록 안내합니다. 도식이나 표를 대신 만들거나 생성이 완료됐다고 말하지 않습니다.
