...
export function LoginChoices({ enabled }: { enabled: boolean }) {
  ...
  if (!enabled) {
    return (
      <div className="rounded-2xl border border-line bg-white p-5 text-sm leading-6 text-ink-soft">
  ...
  return (
    <div>
      <div className="grid gap-3 sm:grid-cols-2">
        <Button size="lg" onClick={() => login("STUDENT")} disabled={Boolean(loading)} className="group justify-between px-4.5">
        ...
        <Button variant="secondary" size="lg" onClick={() => login("ADMIN")} disabled={Boolean(loading)} className="group justify-between px-4.5">
      ...
      {error && <p role="alert" className="mt-3 text-sm text-[#a6383b]">{error}</p>}
    </div>
  );
}
