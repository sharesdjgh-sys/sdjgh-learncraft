...
export async function GET() {
  ...
  if (db) {
    const [summaryResult, trendResult, unitResult, studentResult] = await Promise.all([
      ...
      db.execute(sql`SELECT usr.name, usr.official_grade AS grade, COUNT(*)::int AS requests, sch.daily_ai_limit AS limit, COALESCE(SUM(e.input_tokens + e.output_tokens), 0)::int AS tokens, COALESCE(SUM(e.estimated_cost_usd), 0)::float AS cost FROM usage_events e JOIN users usr ON usr.id = e.student_id JOIN schools sch ON sch.id = e.school_id WHERE e.school_id = ${admin.schoolId}::uuid AND e.status = 'SUCCEEDED' AND e.created_at >= ((now() AT TIME ZONE sch.timezone)::date::timestamp AT TIME ZONE sch.timezone) AND e.created_at < (((now() AT TIME ZONE sch.timezone)::date + 1)::timestamp AT TIME ZONE sch.timezone) GROUP BY usr.id, sch.daily_ai_limit ORDER BY requests DESC LIMIT 20`),
    ]);
