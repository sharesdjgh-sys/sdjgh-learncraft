...
const generatedUnitSchema = z.object({
  ...
  keywords: z.array(z.string().min(1).max(80).describe("학생이 뜻을 물을 만한 교과 개념어·전문용어 또는 관용 표현. 목차 제목이나 학습 활동 설명이 아닌 짧은 명사구")).max(12),
...
export async function generateCourseContent(input: {
  ...
  const commonPrompt = [
    ...
    "각 단원에는 핵심 포인트 3~8개, 예시 1~4개, 추천 질문 3~6개를 포함하세요.",
    "keywords에는 학생이 클릭해 뜻을 물을 실제 교과 개념어·전문용어·관용 표현만 0~12개 작성하세요.",
    "keywords에 과목명, 대단원·중단원·소단원·작품 제목, 인명, 활동명, 학습 목표, 설명 문장, 풀이 지시를 넣지 마세요. 근거 있는 어휘가 없으면 빈 배열로 두세요.",
