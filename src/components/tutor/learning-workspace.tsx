    ...
    <div className="app-enter flex h-dvh min-h-[40rem] flex-col">
      <StudentTopNavigation user={{ name: studentName, schoolName }} actions={(
        <>
          ...
          <button onClick={() => setConceptOpen(true)} className="flex min-h-10 items-center gap-2 rounded-[11px] border border-line bg-surface px-3 text-[.8rem] font-semibold text-brand-dark transition-all duration-300 hover:-translate-y-px hover:border-[var(--line-2)] min-[1340px]:hidden" aria-label="단원 핵심 노트 보기">
      ...
      <aside className="scrollbar-subtle hidden overflow-y-auto border-l border-line bg-surface/52 px-5 py-5 min-[1340px]:block"><ConceptPanel unit={selectedUnit} /></aside>
...
function Sheet({ title, onClose, side = "bottom", children }: { title: string; onClose: () => void; side?: "bottom" | "right"; children: React.ReactNode }) {
  return (
    <div className="fixed inset-0 z-50" role="dialog" aria-modal="true" aria-label={title}>
      <button className="absolute inset-0 cursor-default bg-[#ded9d0]/72 backdrop-blur-[3px]" onClick={onClose} aria-label={`${title} 닫기`} />
      <div className={cn("scrollbar-subtle absolute overflow-y-auto border-line bg-surface shadow-[0_0_60px_rgba(42,35,31,.18)]", side === "right" ? "inset-y-0 right-0 w-[min(26rem,92vw)] border-l px-5 py-5" : "inset-x-0 bottom-0 max-h-[88dvh] rounded-t-[1.35rem] border-t px-5 pb-[calc(5rem+env(safe-area-inset-bottom))] pt-5 sm:left-auto sm:right-0 sm:top-0 sm:max-h-none sm:w-[25rem] sm:rounded-none sm:border-l sm:border-t-0")}>
