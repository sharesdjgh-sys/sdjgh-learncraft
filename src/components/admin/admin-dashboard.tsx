    ...
    <div className="mx-auto max-w-[1480px] px-4 py-8 sm:px-7 lg:px-10 lg:py-12">
      <header className="grid gap-6 border-b border-line pb-8 xl:grid-cols-[1fr_auto] xl:items-end">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="flex rounded-full bg-[#ede7f8] p-1">{ranges.map((item) => <button key={item} onClick={() => setRange(item)} className={cn("min-h-10 rounded-full px-4 text-[.8rem] font-bold transition", range === item ? "bg-white text-brand-dark shadow-[var(--lift-1)]" : "text-ink-4")}>{item}</button>)}</div>
          <select className="min-h-12 rounded-[14px] border border-line bg-white px-3 text-[.82rem] font-bold text-ink"><option>전체 학년</option><option>1학년</option><option>2학년</option><option>3학년</option></select>
          <select className="min-h-12 rounded-[14px] border border-line bg-white px-3 text-[.82rem] font-bold text-ink"><option>전체 과목</option><option>국어</option><option>영어</option><option>수학</option></select>
        </div>
      </header>
      ...
      <section className="mt-9 grid gap-8 xl:grid-cols-[1.4fr_1fr]">
        <article className="rounded-[22px] bg-white p-5 shadow-[var(--lift-2)] sm:p-7">
          <div><h2 className="text-lg font-extrabold">일별 학습 활동</h2><p className="mt-1 text-[.82rem] text-ink-4">성공한 AI 요청 추세</p></div>
          <div className="mt-7 h-[280px] w-full"><ResponsiveContainer><AreaChart data={metrics.dailyTrend} margin={{ left: -20, right: 5 }}><defs><linearGradient id="activityFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#7b5cf0" stopOpacity={0.28} /><stop offset="100%" stopColor="#7b5cf0" stopOpacity={0.02} /></linearGradient></defs><CartesianGrid stroke="rgba(126,101,181,.13)" vertical={false} /><XAxis dataKey="date" axisLine={false} tickLine={false} tick={{ fill: "#8b80a8", fontSize: 12 }} /><YAxis axisLine={false} tickLine={false} tick={{ fill: "#8b80a8", fontSize: 12 }} /><Tooltip contentStyle={{ borderRadius: 14, border: "1px solid rgba(126,101,181,.18)", boxShadow: "0 10px 30px rgba(83,61,130,.1)" }} /><Area type="monotone" dataKey="requests" stroke="#7b5cf0" strokeWidth={2.5} fill="url(#activityFill)" /></AreaChart></ResponsiveContainer></div>
        </article>
        <article className="rounded-[22px] bg-white p-5 shadow-[var(--lift-2)] sm:p-7">
          <div><h2 className="text-lg font-extrabold">질문 집중 단원</h2><p className="mt-1 text-[.82rem] text-ink-4">보완 수업이 필요한 단원 순위</p></div>
          <div className="mt-7 h-[280px] w-full"><ResponsiveContainer><BarChart data={metrics.unitRanking} layout="vertical" margin={{ left: 6, right: 12 }}><CartesianGrid stroke="rgba(126,101,181,.1)" horizontal={false} /><XAxis type="number" hide /><YAxis dataKey="unit" type="category" axisLine={false} tickLine={false} width={108} tick={{ fill: "#6b5f8a", fontSize: 11.5 }} /><Tooltip cursor={{ fill: "#faf8fe" }} contentStyle={{ borderRadius: 14, border: "1px solid rgba(126,101,181,.18)" }} /><Bar dataKey="requests" fill="#c2416b" radius={[0, 8, 8, 0]} barSize={16} /></BarChart></ResponsiveContainer></div>
        </article>
      </section>

      <section className="mt-9 overflow-hidden rounded-[22px] bg-white shadow-[var(--lift-2)]">
        <div className="flex items-center justify-between border-b border-line px-5 py-5 sm:px-7"><div><h2 className="text-lg font-extrabold">학생별 사용량</h2><p className="mt-1 text-[.82rem] text-ink-4">질문 원문은 수집하거나 표시하지 않습니다.</p></div><button className="flex min-h-10 items-center gap-1.5 rounded-full px-3 text-[.82rem] font-bold text-brand hover:bg-brand-soft">전체 보기 <ArrowUpRight size={15} /></button></div>
