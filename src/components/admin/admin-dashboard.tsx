    ...
    <div className="mx-auto max-w-[1480px] px-4 py-8 sm:px-7 lg:px-10 lg:py-12">
      ...
      <section className="mt-9 grid gap-8 xl:grid-cols-[1.4fr_1fr]">
        <article className="rounded-[16px] border border-line bg-surface p-5 shadow-[var(--lift-2)] sm:p-7">
          ...
          <div className="mt-7 h-[280px] w-full"><ResponsiveContainer><AreaChart data={metrics.dailyTrend} margin={{ left: -20, right: 5 }}><defs><linearGradient id="activityFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#6847e8" stopOpacity={0.22} /><stop offset="100%" stopColor="#6847e8" stopOpacity={0.01} /></linearGradient></defs><CartesianGrid stroke="rgba(48,52,91,.1)" vertical={false} /><XAxis dataKey="date" axisLine={false} tickLine={false} tick={{ fill: "#6f768f", fontSize: 12 }} /><YAxis axisLine={false} tickLine={false} tick={{ fill: "#6f768f", fontSize: 12 }} /><Tooltip contentStyle={{ borderRadius: 10, border: "1px solid rgba(48,52,91,.14)", boxShadow: "0 12px 28px rgba(46,43,90,.09)" }} /><Area type="monotone" dataKey="requests" stroke="#6847e8" strokeWidth={2.5} fill="url(#activityFill)" /></AreaChart></ResponsiveContainer></div>
        </article>
        <article className="rounded-[16px] border border-line bg-surface p-5 shadow-[var(--lift-2)] sm:p-7">
          <div><h2 className="text-lg font-extrabold">질문 집중 단원</h2><p className="mt-1 text-[.82rem] text-ink-4">보완 수업이 필요한 단원 순위</p></div>
          <div className="mt-7 h-[280px] w-full"><ResponsiveContainer><BarChart data={metrics.unitRanking} layout="vertical" margin={{ left: 6, right: 12 }}><CartesianGrid stroke="rgba(48,52,91,.08)" horizontal={false} /><XAxis type="number" hide /><YAxis dataKey="unit" type="category" axisLine={false} tickLine={false} width={108} tick={{ fill: "#5c627d", fontSize: 11.5 }} /><Tooltip cursor={{ fill: "#f7f7fc" }} contentStyle={{ borderRadius: 10, border: "1px solid rgba(48,52,91,.14)" }} /><Bar dataKey="requests" fill="#6847e8" radius={[0, 5, 5, 0]} barSize={16} /></BarChart></ResponsiveContainer></div>
        </article>
      </section>
