import { AlertCircle, BookOpenText, CircleHelp, Lightbulb, Sparkles } from "lucide-react";
  ...
  return (
    <section aria-label={`${card.term} 어휘 설명`} className="my-4 overflow-hidden rounded-2xl border border-brand/20 bg-surface shadow-[var(--lift-1)]">
      <header className="relative overflow-hidden bg-[linear-gradient(135deg,var(--brand-page),var(--surface))] px-5 py-5 sm:px-6">
        ...
        <p className="relative mt-3 max-w-3xl text-[.95rem] font-semibold leading-7 text-brand-dark sm:text-base">{card.oneLineMeaning}</p>
      </header>
      ...
      <div className="grid gap-px bg-line sm:grid-cols-2">
        <article className="bg-surface px-5 py-4 sm:px-6">
          ...
          <p className="mt-2 text-[.88rem] leading-6 text-ink-2">{card.story}</p>
        </article>
        <article className="bg-surface px-5 py-4 sm:px-6">
          <p className="text-[.75rem] font-extrabold text-ink-3">단원 속 한 문장</p>
          <p className="mt-2 rounded-xl bg-brand-soft/55 px-3.5 py-3 text-[.88rem] font-bold leading-6 text-ink">“{card.example.sentence}”</p>
          <p className="mt-2 text-[.78rem] leading-5 text-ink-4">여기서는 {card.example.meaning}</p>
        </article>
      </div>
        ...
        <div className="grid gap-2 border-t border-line bg-surface-2 px-4 py-4 sm:grid-cols-2 sm:px-5">
          {card.memoryCue && (
            <div className="rounded-xl border border-[#eadfb8] bg-[#fffaf0] px-4 py-3">
              ...
              <p className="mt-1.5 text-[.8rem] leading-5 text-ink-3">{card.memoryCue}</p>
            </div>
          )}
          {card.caution && (
            <div className="rounded-xl border border-[#ecd6d1] bg-[#fff7f5] px-4 py-3">
              ...
              <p className="mt-1.5 text-[.8rem] leading-5 text-ink-3">{card.caution}</p>
            </div>
          )}
        </div>
      )}

      <footer className="flex gap-3 border-t border-brand/15 bg-brand-page px-5 py-4 sm:px-6">
        ...
        <div>
          ...
          <p className="mt-1 text-[.84rem] font-semibold leading-6 text-ink-2">{card.quickCheck}</p>
        </div>
      </footer>
    </section>
  );
